...
export const getAddressInfo = (addr: string): Promise<ApiResponse> => {
  const cachedResponse = cache.get(addr) as ApiResponse;
  ...
  return axios
    .get(`${API_ENDPOINT}/${addr}`)
    .then(({ data }) => cache.put<ApiResponse>(addr, data, 3600 * 1000))
